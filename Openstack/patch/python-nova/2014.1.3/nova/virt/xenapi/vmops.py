...
class VMOps(object):
    ...
    def _start(self, instance, vm_ref=None, bad_volumes_callback=None):
        ...
        resp = self._session.call_xenapi('VM.start_on', vm_ref,
    ...
    def _spawn(self, context, instance, image_meta, step, create_disks_step,
        ...
        def boot_instance_step(undo_mgr, vm_ref):
            if power_on:
                # Abhay Dandekar
                # DCG changes 
                res = self._start(instance, vm_ref)
                """ MH start of throw exception, if RPCORE denies VM launch"""
                if res:
                    msg = _("RPCORE DENIED VM LAUNCH")
