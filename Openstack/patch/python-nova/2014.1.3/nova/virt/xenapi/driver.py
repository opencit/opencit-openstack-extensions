...
xenapi_opts = [
    ...
    cfg.StrOpt('xenapi_proxy_connection_url',
               deprecated_name='xenapi_proxy_connection_url',
               deprecated_group='DEFAULT',
               help='URL for connection to proxy xmlrpc server on XenServer. '
                    'Needed for Hadoop on openstack project for measured launch'),
...
class XenAPIDriver(driver.ComputeDriver):
    ...
    def __init__(self, virtapi, read_only=False):
        ...
        url = CONF.xenapi_proxy_connection_url
        username = CONF.xenserver.connection_username
        password = CONF.xenserver.connection_password
        if not url or password is None:
            raise Exception(_('Must specify xenapi_proxy_connection_url, '
    ...
    def spawn(self, context, instance, image_meta, injected_files,
        ...
        LOG.debug("mhagent hook: Spawning instance: instance=%s, image_meta=%s, block_device_info=%s", jsonutils.dumps(instance), jsonutils.dumps(image_meta), jsonutils.dumps(block_device_info))
        if 'properties' in image_meta and 'mh_encrypted' in image_meta['properties']:
            LOG.debug("mhagent hook: spawn encrypted image, original checksum=%s", image_meta['properties']['mh_checksum'])
            ...
            LOG.debug("mhagent hook: spawn plain image")
        # jbuhacoff end
