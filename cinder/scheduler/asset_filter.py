...
trusted_opts = [
    ...
    cfg.StrOpt('attestation_port', default='8443',
    ...
    cfg.IntOpt('attestation_auth_timeout', default=60,
               help='Attestation status cache valid period length'),
    cfg.StrOpt('asset_tag_server', help='asset tag server http'),
    cfg.StrOpt('asset_tag_server_port', default='9999',
               help='asset tag server port'),
    cfg.StrOpt('asset_tag_server_url', default='/selections',
...
class AssetTagService(object):
    ...
    def __init__(self):
        ...
        self.auth_blob = \
            CONF.trusted_computing.asset_tag_server_auth_blob
    ...
    def get_selection_details(self, selection_id):
        ...
        c.request('GET', '/selections?id=' + selection_id + '&'
                  + str(random.random()), headers=headers)
        res = c.getresponse()
        res_data = res.read()
        ...
        for tag in tags:
            if tag['tagName'].lower() not in tag_dictionary:
                ...
                tag_dictionary[tag['tagName'
                               ].lower()].append(tag['tagValue'])

        return tag_dictionary


class AssetFilter(filters.BaseHostFilter):
    ...
    def host_passes(self, host_state, filter_properties):
        ...
        #spec = json.loads(filter_properties['volume_properties'])
        spec = filter_properties.get('request_spec', {})
        volume_props = spec.get('volume_properties', {}).get('metadata', {})


    # Get the Tag verification flag from the image properties
    #changes here
        tag_selections = volume_props.get('tags')  # comma seperated values
        trust_verify = volume_props.get('trust')  # comma seperated values
	...
	host_data = \
            self.attestservice.do_attestation(host_state.host)
        LOG.error(host_data)
    ...
    def verify_and_parse_saml(self, saml_data):
        ...
        try:
            ...
            xp_attributestatement = \
                '{saml2}AttributeStatement/{saml2}Attribute'.format(**ns)
            ...
            for el in elements:
                if el.attrib['Name'].lower() == 'trusted':
                    ...
                    asset_tag[el.attrib['Name'].lower().split('['
                              )[1].split(']')[0]] = \
                        el.find(xp_attributevalue).text
    ...
    def verify_asset_tag(self, host_tags, tag_selections):
        ...
        for tag in list(sel_tags.keys()):
            if tag not in list(host_tags.keys()) or host_tags[tag] \
                not in sel_tags[tag]:
