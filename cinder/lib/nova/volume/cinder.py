...
class API(object):
    ...
    def check_vol_instance_tags(self, volume, instance):
        #LOG.error("------------------------------------------------")
        #LOG.error("Attaching a volume\n Country whre its at ")
        #LOG.error("---++++++---")
        #LOG.error(volume['volume_metadata']['tags'])#properties earlier
        volume_tags=volume['volume_metadata']['tags']
        #LOG.error(volume_tags)
        #string=json.loads(stuff)
        #string =instance['metadata']
        #LOG.error("String value : ")
        #LOG.error(string['country'])
        #LOG.error("---+++++++++++++++++++++++----")
        #LOG.error(volume)
        #LOG.error("------------------------------------------------")
        #LOG.error("Instance details : ---------------------");
        #LOG.error(instance['system_metadata']['image_trust'])
        #LOG.error(instance['system_metadata']['image_tags'])
        instance_tags=instance['system_metadata']['image_tags']
        #LOG.error(instance_tags)
      
        if json.loads(volume_tags) == json.loads(instance_tags):
            LOG.info("Volume and instance tags match.")
	    return True
        LOG.error("Volume and instance tags do not match.")
    ...
    def check_attach(self, context, volume, instance=None):
        ...
        if instance and not CONF.cinder_cross_az_attach:
            if instance['availability_zone'] != volume['availability_zone']:
                ...
                raise exception.InvalidVolume(reason=msg)
        if(not self.check_vol_instance_tags(volume, instance)):
                msg = _("Instance and volume does not belong to the same geo")
                raise exception.InvalidVolume(reason=msg)
