...
maven_get_vrtm() {
  local _group_id="com.intel.mtwilson.vrtm.packages"
  local _artifact_id="vrtm"
  local _version="${CIT_VERSION}"
  local _packaging="bin"
  local _classifier=
  local _destination=
  ...
  # UBUNTU
    ...
    echo "Unsupported OS: Cannot retrieve vrtm package"
    exit 1
  fi
  _destination="$(pwd)/${_artifact_id}-${_version}-${_classifier}.${_packaging}"
  maven_get "${_group_id}" "${_artifact_id}" "${_version}" "${_packaging}" "${_classifier}" "${_destination}"
