...
class TrustAssertionFilter(filters.BaseHostFilter):
    ...
    def verify_and_parse_saml(self, saml_data):
        ...
        try:
            ...
            LOG.debug("System exists in the Mt. Wilson portal")
        ...
        try:
            ...
            for el in elements:
                if el.attrib['Name'].lower() == 'trusted':
                    ...
                    if el.attrib['Name'].lower().split('[')[1].split(']')[0].lower() not in asset_tag:
                        asset_tag[el.attrib['Name'].lower().split('[')[1].split(']')[0].lower()] = []
                 
                    asset_tag[el.attrib['Name'].lower().split('[')[1].split(']')[0].lower()].append(el.find(xp_attributevalue).text.lower())
            ...
            LOG.error("Excepiton in verify_and_parse_saml method")
            LOG.debug(e)
    ...
    def verify_asset_tag(self, host_tags, tag_selections):
        ...
        try: 
            ...
            for tag in list(sel_tags.keys()):
                LOG.debug(tag)
                LOG.debug(len(list(set(host_tags[tag]) & set(sel_tags[tag]))))
                if tag not in list(host_tags.keys()) or len(list(set(host_tags[tag]) & set(sel_tags[tag]))) == 0:
